...
<script>
    ...
    function initChatRoom() {
        ...
        function onQueueItemConfirmed(tempId, storagePath) {
            ...
            // Swap "(sending image...)" placeholder → real image
            if (storagePath) {
                swapBubbleToConfirmed(tempId, storagePath, item?.text ?? '');
            }

            // Remove optimistic bubble after a short delay
            // (gives morph time to paint the real message first)
            setTimeout(() => {
        ...
        Livewire.on('message-confirmed', ({ tempId, storagePath }) => {
            onQueueItemConfirmed(tempId, storagePath ?? null);
        });
        ...
        // No optimistic cleanup needed here — #optimisticMessages is wire:ignore,
        // so Livewire never touches it. Bubbles are removed directly in onQueueItemConfirmed.
